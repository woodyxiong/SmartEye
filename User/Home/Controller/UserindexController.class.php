...
class UserindexController extends Controller{
	public function userindex(){
		...
		// 导出数据
		$camera=M('camera')->field('cameraid,cameraname,longitude,latitude,time')->where("userid='".$userid."'")->select();
		foreach ($camera as $a => $b) {
			if ($a%4==0){
				...
				$camera[$a-1]['time1']=$camera[$a]['time'];
				unset($camera[$a]);
			}elseif ($a%4==2) {
				...
				$camera[$a-2]['time2']=$camera[$a]['time'];
				unset($camera[$a]);
			}elseif ($a%4==3) {
				...
				$camera[$a-3]['time3']=$camera[$a]['time'];
				unset($camera[$a]);
			}
		}
